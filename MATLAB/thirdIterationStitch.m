...
for imN = startImage:(startImage+numToStitch)-1
    filename = sprintf('tiger/im%d.jpeg', imN); % defining the filename
    im = imresize(im,0.9); %- required for barret1 images (change the number to 2)
    ...
    for n = 2:(numToStitch/2) + 1
        ...
        while (numMatches > 1600)    
            if (j == 1)
                ...
                filename = sprintf('tiger/im%d.jpeg', N); % defining the filename
                im = imread(filename); % reading the image from the given filename
